...
-- 컴퓨터 공학부에 2022년 홍길동 교수님이 오셨다. 이 때 홍길동 교수님의 정보를 추가하는 작업을 해보새요.
-- 이 때 교수님 학번은 2022160001이 할당 되어야 합니다.
-- 교수 정보를 추가 하기 위한 프로시저(입학년도, 이름, 학부명, 교수직위, 전화번호)
...
-- 학생정보 추가를 위한 프로시저(신입생 추가)
-- 2022년에 고길동 학생이 컴퓨터 공학부에 입학하였다. 지도 교수님은 같은과 홍길동 교수님
...
BEGIN
		declare _dep_num int;
        declare _studnet_count int default 0;
        ...
        if _dep_num is not null then
            ...
            set _student_num = concat(_year, _dep_num, lpad(_studnet_count,3,'0'));
            set _professor_num =
...
-- 성적이 계산되어 총점에 저장
-- 학점은 100이하 90 이상 A, 90미만 80이상 B, 80미만 70이상 C, 70미만 60이상 D, 나머지 F
...
-- 2022년 1학기 자바를 수강하는 고길동 학생의 성적을 등록하려고 한다.
-- 중간은 100, 기말은 80, 출석 100, 과제 90점이고, 비율은 중간 4, 기말 4, 출석 1, 과제 1로
-- 성적이 계산되어 총점에 저장
-- 학점은 100이하 90 이상 A, 90미만 80이상 B, 80미만 70이상 C, 70미만 60이상 D, 나머지 F
